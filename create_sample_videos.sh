#!/bin/bash
# Create sample video and thumbnail files for testing
# These are placeholder files - replace with real videos

echo "Creating sample video files..."

# Create minimal AVI video files (these are placeholder - real videos needed)
VIDEOS=(
...
for video in "${VIDEOS[@]}"; do
    echo "Creating $video..."
    # Create minimal MP4 file header (real video needed for playback)
    printf 'RIFF\x00\x00\x00\x00AVI \x00\x00\x00\x00' > "/home/prosper/itechtube/storage/app/public/videos/$video"
    # Add some dummy data
    dd if=/dev/zero bs=1024 count=100 >> "/home/prosper/itechtube/storage/app/public/videos/$video" 2>/dev/null
done

# Create thumbnail files (placeholder images)
for thumb in "${THUMBNAILS[@]}"; do
    echo "Creating $thumb..."
    # Create minimal JPEG file
    printf '\xff\xd8\xff\xe0\x00\x10JFIF\x00\x01\x01\x00\x00\x01\x00\x01\x00\x00' > "/home/prosper/itechtube/storage/app/public/thumbnails/$thumb"
    # Add some dummy data
    dd if=/dev/zero bs=1024 count=10 >> "/home/prosper/itechtube/storage/app/public/thumbnails/$thumb" 2>/dev/null
done
...
echo ""
echo "Files in thumbnails folder:"
ls -la /home/prosper/itechtube/storage/app/public/thumbnails/
echo ""
echo "IMPORTANT: These are placeholder files. For real video playback,"
echo "copy your actual .avi/.mp4 video files to:"
echo "  /home/prosper/itechtube/storage/app/public/videos/"
echo ""
echo "And copy actual images to:"
